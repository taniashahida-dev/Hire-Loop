      ...
      <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="h-20 flex items-center justify-between">
          ...
          <Link href="/" className="flex items-center gap-3">
           <Image src={'/images/logo.png'} alt='logo'  width={140}
    height={40}
    priority
            ...
            <div className="flex items-center gap-1 rounded-2xl border border-white/10 bg-white/5 backdrop-blur-md px-2 py-2">
              {navLinks.map((link) => (
                <Navlink
                  key={link.name}
                  href={link.href}
                  className={"text-sm text-gray-300 hover:text-white px-4 py-2 rounded-xl transition-all duration-200 hover:bg-white/10"}
                >
                  {link.name}
               </Navlink>
              ))}
